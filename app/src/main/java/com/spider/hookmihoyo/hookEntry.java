...
public class hookEntry implements IRposedHookLoadPackage {
    ...
    public void handleLoadPackage(RC_LoadPackage.LoadPackageParam lpparam) throws Throwable {
        Log.d(TAG, "handleLoadPackage: hook start");
        if (lpparam.packageName.equals("com.mihoyo.hyperion")) {
            ...
            RposedHelpers.findAndHookMethod("java.lang.Runtime", lpparam.classLoader, "loadLibrary0", Class.class, String.class, new RC_MethodHook() {
                ...
                protected void afterHookedMethod(MethodHookParam param) throws Throwable {
                    ...
                    if (param.args[1].equals("anative")) {
                        loadSoFile("librate.so", "content://com.spider.hookmihoyo/assets/jniLibs/arm64-v8a/libratelhookdemo.so");
                    }
                }
            });
        }
    ...
    private void loadSoFile(String newFileName, String soDir) {
        try {
            ...
            String nativeLibraryDir = sContext.getApplicationInfo().nativeLibraryDir;
            Log.d(TAG, "loadSoFile: nativeDir = " + nativeLibraryDir);
        } catch (Exception e) {
